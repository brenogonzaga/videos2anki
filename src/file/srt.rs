...
pub fn sentences_and_times(reader: BufReader<File>) -> (Vec<String>, Vec<(String, String)>) {
    let re_sentences = Regex::new(r"\D(.+)").unwrap();
    let re_times = Regex::new(r"(\d{2}:\d{2}:\d{2},\d{3}) --> (\d{2}:\d{2}:\d{2},\d{3})").unwrap();
